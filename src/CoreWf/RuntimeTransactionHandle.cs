...
namespace CoreWf
{
    ...
    public sealed class RuntimeTransactionHandle : Handle, IExecutionProperty, IPropertyRegistrationCallback
    {
        ...
        public RuntimeTransactionHandle(Transaction rootTransaction)
        {
            ...
            {
                throw CoreWf.Internals.FxTrace.Exception.ArgumentNull("rootTransaction");
            }
        ...
        void ThrowIfRegistered(string message)
        {
            ...
            {
                throw CoreWf.Internals.FxTrace.Exception.AsError(new InvalidOperationException(message));
            }
        }
        ...
        Transaction GetCurrentTransactionCore(ActivityContext context)
        {
            ...
            {
                throw CoreWf.Internals.FxTrace.Exception.AsError(new InvalidOperationException(SR.UnInitializedRuntimeTransactionHandle));
            }
        ...
        void RequestOrRequireTransactionContextCore(NativeActivityContext context, Action<NativeActivityTransactionContext, object> callback, object state, bool isRequires)
        {
            ...
            {
                throw CoreWf.Internals.FxTrace.Exception.AsError(new InvalidOperationException(SR.RuntimeTransactionAlreadyExists));
            }

            if (context.IsInNoPersistScope)
            {
                throw CoreWf.Internals.FxTrace.Exception.AsError(new InvalidOperationException(SR.CannotSetRuntimeTransactionInNoPersist));
            }
            ...
            if (this.SuppressTransaction)
            {
                throw CoreWf.Internals.FxTrace.Exception.AsError(new InvalidOperationException(SR.RuntimeTransactionIsSuppressed));
            }
            ...
            if (isRequires)
            {
                ...
                {
                    throw CoreWf.Internals.FxTrace.Exception.AsError(new InvalidOperationException(SR.OnlyOneRequireTransactionContextAllowed));
                }
        ...
        void CompleteTransactionCore(NativeActivityContext context, BookmarkCallback callback)
        {
            ...
            if (this.rootTransaction != null)
            {
                throw CoreWf.Internals.FxTrace.Exception.AsError(new InvalidOperationException(SR.CannotCompleteRuntimeOwnedTransaction));
            }

            if (!context.HasRuntimeTransaction)
            {
                throw CoreWf.Internals.FxTrace.Exception.AsError(new InvalidOperationException(SR.NoRuntimeTransactionExists));
            }
